...
const AIChatbot = ({ onGenerateElements, labelSize, generateId }) => {
    ...
    if (!isOpen) {
        return (
            <button
                ...
                className="fixed bottom-8 right-8 w-16 h-16 bg-[var(--color-primary)] text-white rounded-2xl shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-[150] group"
            >
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="p-6 border-b flex items-center justify-between bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-hover)] text-white" style={{ borderColor: theme.border }}>
                    ...
                    >
                        {messages.map((msg, i) => (
                            ...
                            >
                                <div
                                    ...
                                    style={{
                                        backgroundColor: msg.role === 'user' ? undefined : theme.surface,
                                        borderColor: msg.role === 'user' ? 'transparent' : theme.border,
                                        color: msg.role === 'user' ? 'white' : theme.text
                                    }}
                                >
                                    {msg.role === 'assistant' && msg.type !== 'system' && (
                                        <div className="flex items-center space-x-2 mb-2 text-[var(--color-primary)]">
                                            <Terminal size={14} />
                        ...
                        {isTyping && (
                            <div className="flex justify-start">
                                <div
                                    className="p-4 rounded-2xl border rounded-tl-none flex items-center space-x-2 shadow-sm"
                                    style={{ backgroundColor: theme.surface, borderColor: theme.border }}
                                >
                                    <Zap size={14} className="text-[var(--color-primary)] animate-bounce" />
                    ...
                    <div className="p-6 border-t" style={{ borderColor: theme.border }}>
                        <div className="relative group">
                            <input
                                ...
                                style={{ backgroundColor: theme.bg, color: theme.text, borderColor: 'transparent' }}
                            />
                            <button
                                onClick={handleSend}
                                disabled={!input.trim()}
                                className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-[var(--color-primary)] text-white rounded-xl flex items-center justify-center hover:scale-105 active:scale-90 transition-all disabled:opacity-50 disabled:scale-100"
                            >
                                <Send size={18} />
                            </button>
                        </div>
